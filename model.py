...
class SE(Layer):
    def __init__(self, in_channels, channels, se_ratio=12):
        ...
        self.fc = Sequential([
            Conv2D(filters=channels // se_ratio, kernel_size=(1, 1), strides=1, padding='same'),
            BatchNormalization(),
            tf.nn.relu(),
            Conv2D(filters=channels, kernel_size=(1, 1), strides=1, padding='same'),
            sigmoid()
        ])
...
class ReXNetV1(tf.keras.Model):
    ...
    def call(self, x, training=None):
        ...
        x = self.output(x, training=training)
        return x